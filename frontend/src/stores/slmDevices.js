...
const STORAGE_KEY = 'smartam-slm-devices-v2'
const BACKEND_DEVICES_ENDPOINT = '/api/slm/device-group/devices'
...
<svg xmlns="http://www.w3.org/2000/svg" width="${THUMBNAIL_WIDTH}" height="${THUMBNAIL_HEIGHT}" viewBox="0 0 ${THUMBNAIL_WIDTH} ${THUMBNAIL_HEIGHT}">
  ...
  <text x="54" y="206" font-family="Arial, sans-serif" font-size="13" fill="#cbd5e1">SLM-${serial}</text>
</svg>`
...
const normalizeDevice = (device, index) => {
  ...
  return {
    ...
    source: device.source || (device.dataTag ? '7103' : 'manual')
  }
}
...
export const useSlmDeviceStore = defineStore('slmDevices', () => {
  ...
  const loadDevicesFromBackend = async () => {
    ...
    try {
      ...
      const backendDevices = payload.devices.map((device, index) => normalizeDevice({ ...device, source: device.source || '7103' }, index))
